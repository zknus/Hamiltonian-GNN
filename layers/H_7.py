...
class myLinear_W(nn.Module):  
    def __init__(self, size_in):
        super().__init__()
        self.dim = size_in
        self.linear1 = nn.Linear(in_features=2*self.dim,out_features=4*self.dim)
        self.linear2 = nn.Linear(in_features=4*self.dim,out_features=2*self.dim)
